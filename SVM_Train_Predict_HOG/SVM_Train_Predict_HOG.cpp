...
int main()
{
	...
	if(TRAIN)
	{
		...
		for(int num=0; num<PosSamNO && getline(finPos,ImgName); num++)
		{
			...
			ImgName = "C:\\Users\\zzy19\\Documents\\GitHub\\INRIAPerson\\96X160H96\\Train\\pos\\" + ImgName;//加上正样本的路径名
	...
	{
		svm = StatModel::load<SVM>("SVM_HOG.xml");//从XML文件读取训练好的SVM模型
	...
	***************************************************************************************************/
	...
	Mat supportVectorMat = svm->getSupportVectors();//supportVectorMat元素的数据类型为float
	const int supportVectorNum = supportVectorMat.rows;//支持向量的个数
	cout<<"支持向量个数："<<supportVectorNum<<endl;
	Mat alphaMat, supportVectorIndex;
	alphaMat = Mat::zeros(supportVectorNum, DescriptorDim, CV_32F);//alphamat和svindex必须初始化，否则getDecisionFunction()函数会报错
	supportVectorIndex = Mat::zeros(1, supportVectorNum, CV_64F);
	//alpha向量，长度等于支持向量个数
	//supportVectorIndex,支持向量矩阵
	...
	//Mat testFeatureMat = Mat::zeros(1,3780,CV_32SC1);//测试样本的特征向量矩阵
